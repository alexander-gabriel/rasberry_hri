...
from opencog.type_constructors import set_type_ctor_atomspace, NumberNode, \
                                      ConceptNode
...
from parameters import NO_BERRY_PLACES, ROBOT_WIDTH, ROBOT_LENGTH, \
                       PICKER_WIDTH, PICKER_LENGTH, TARGET_PICKER, \
                       CALLED_ROBOT, SEEN_PICKING, HAS_CRATE, CRATE_FULL, \
                       MIN_GAIN, MAX_COST, MINIMUM_DISTANCE, \
...
class BDISystem:
    ...
    def __init__(self, me, kb):
        ...
        with self.kb.lock:
            ...
            self.picker_pose_publisher = rospy.Publisher('/picker_mover',
            ...
            #     nodes = link.split("_")
            #     self.links[link] = self.world_state.get_distance(nodes[0], nodes[1])
            self.setup_experiment(me)
        rospy.loginfo("BDI: Initialized BDI System")

    def setup_experiment(self, me):
        ...
        for goal in self.desires:
            if goal not in self.intentions:
                ...
                for args in args_list:
                    if len(args) > 0:
                        try:
                            intention_candidates.append(goal(
                                        self.world_state, self.robco, args))
                        except WrongParameterException as err:
                            rospy.logerr(
                                "BDI: Couldn't add intention {:}".format(err))
                            # pass
        for intention in self.intentions:
            if intention in intention_candidates \
                    and intention.is_achieved(self.world_state):
    ...
    def filter_intention_candidates(self, intention_candidates):
        ...
        for desire in intention_candidates:
            ...
            if gain > MIN_GAIN and cost < MAX_COST:
                ...
                rospy.logwarn(("BDI: 149 - Desire {} is outside bounds "
                               "of gain >{} and cost <{}").format(desire,
                                                                  MIN_GAIN,
                                                                  MAX_COST))
    ...
    def perform_action(self):
        ...
        for intention in self.intentions:
            ...
            with suppress(AttributeError):
                ...
                rospy.logdebug(("BDI: Action option {:} "
                                "with cost {:.2f}").format(action, cost))
                if cost < min_cost:
    ...
    def loop(self):
        ...
        self.update_beliefs()
        rospy.logdebug("BDI: --  updated beliefs   -- {:.4f}".format(
                                                    time.time() - start_time))
        start_time = time.time()
        intention_candidates = self.generate_intention_candidates()
        rospy.logdebug("BDI: -- generated desires  -- {:.4f}".format(
                                                    time.time() - start_time))
        start_time = time.time()
        self.intentions = self.filter_intention_candidates(
                                                        intention_candidates)
        rospy.logdebug("BDI: -- filtered desires   -- {:.4f}".format(
                                                    time.time() - start_time))
        start_time = time.time()
        self.perform_action()
        rospy.logdebug("BDI: -- performed action   -- {:.4f}".format(
                                                    time.time() - start_time))
        start_time = time.time()
        self.clean_intentions()
        rospy.logdebug("BDI: -- cleaned intentions -- {:.4f}".format(
                                                    time.time() - start_time))
    ...
    def clean_intentions(self):
        ...
        while index < len(self.intentions):
            if self.intentions[index].is_achieved(self.world_state):
                rospy.loginfo("BDI: Finished following {}".format(
                                                    self.intentions[index]))
                # self.robco.move_to(INITIAL_WAYPOINT)
                del self.intentions[index]
            else:
                index += 1

    def calculate_directions(self, world, pairs):
        ...
        try:
            ...
            for k, v in qsrlib_response_message.qsrs.trace[t].qsrs.items():
                ...
                try:
                    update_direction = self.latest_directions[picker.name] \
                                       != direction
                except Exception:
                ...
                if update_direction:
                    if direction == "+":
                        ...
                        rospy.logwarn("BDI: {} is approaching".format(
                                                                picker.name))
                    else:
                        if self.world_state.approaching(picker).tv \
                                == self.kb.TRUE:
                        ...
                        rospy.logwarn("BDI: {} is standing".format(
                                                                picker.name))
                    self.latest_directions[picker.name] = direction
        # except (KeyError, AttributeError) as err:
        #     rospy.logwarn("BDI: 242 - Key or Attribute error: {}".format(err))
        ...
        try:
            ...
            minimum_distance = \
                max(MINIMUM_DISTANCE,
                    self.world_state.get_optimum_distance(person))

            if distance < minimum_distance:
                if not self.too_close:
                    rospy.logwarn(("BDI: Robot has met picker. "
                                   "Distance: {}").format(distance))
                    self.too_close = True
                    self.picker_pose_publisher.publish("at robot")
                    self.robco.cancel_movement()
                elif distance < 0.2:
                    rospy.logwarn(("BDI: Picker is too close. "
                                   "Distance: {}").format(distance))
            ...
            rospy.logerr("BDI: 268 - Couldn't react to distance events. Error: {:}".format(err))
            raise err
        ...
        try:
            ...
            if current_node != "none":
                ...
                if current_node != latest_node:
                    self.world_state.update_position(person,
                                                     ConceptNode(current_node))
            elif closest_node is None:
                rospy.logwarn(("BDI: We have no idea "
                               "where {} is currently").format(person.name))
            ...
            rospy.logerr(("BDI - Couldn't update picker node. "
                          "Error: {:}").format(err))
    ...
    def handle_position_msgs(self):
        ...
        if self.latest_robot_msg is not None:
            self.world_state.set_position(self.me,
        ...
        for person, msg in self.latest_people_msgs.items():
            ...
            self.world_state.set_position(person, msg.pose.position.x,
                                          msg.pose.position.y, timestamp)
            ...
            position = Object_State(name=person.name, timestamp=timestamp,
                                    x=msg.pose.position.x,
                                    y=msg.pose.position.y,
                                    xsize=PICKER_WIDTH, ysize=PICKER_LENGTH,
                                    object_type="Person")
            try:
        ...
        if pairs and self.latest_robot_msg is not None:
            self.robot_track.append(Object_State(
                                        name=self.me.name, timestamp=timestamp,
                                        x=self.latest_robot_msg.position.x,
                                        y=self.latest_robot_msg.position.y,
                                        xsize=ROBOT_WIDTH, ysize=ROBOT_LENGTH))
